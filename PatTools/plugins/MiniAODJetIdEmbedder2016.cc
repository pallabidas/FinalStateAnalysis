...
class MiniAODJetIdEmbedder2016 : public edm::stream::EDProducer<edm::GlobalCache<ONNXRuntime>> {
  public:
    MiniAODJetIdEmbedder2016(const edm::ParameterSet& pset, const ONNXRuntime *);
...
int center_norm_pad_2016(const std::vector<float> &input,
                    float center,
                    float norm_factor,
                    unsigned min_length,
                    unsigned max_length,
...
MiniAODJetIdEmbedder2016::MiniAODJetIdEmbedder2016(const edm::ParameterSet& pset, const ONNXRuntime *cache) {
...
std::unique_ptr<ONNXRuntime> MiniAODJetIdEmbedder2016::initializeGlobalCache(const edm::ParameterSet &iConfig) {
  return std::make_unique<ONNXRuntime>(iConfig.getParameter<edm::FileInPath>("model_path").fullPath());
}

void MiniAODJetIdEmbedder2016::globalEndJob(const ONNXRuntime *cache) {}
    ...
    if(jet.pt() > 10. && abs(jet.eta()) < 2.4) {
      ...
      for (unsigned igroup = 0; igroup < input_names_.size(); ++igroup) {
        ...
        for (unsigned i = 0; i < prep_params.var_names.size(); ++i) {
          ...
          int insize = center_norm_pad_2016(raw_value,
                                       info.center,
                                       info.norm_factor,
